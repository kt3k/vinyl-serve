...
var VinylServer = subclass(function (pt) {
    ...
    pt.createDebugPageHtml = function () {
        ...
        buf += '<h1>Welcome to ' + pkg.name + ' debug page!</h1>'

        buf += '<h2>Available paths (' + assetKeys.length + ')</h2>'
        ...
        return buf + this.getSignatureHtml() + this.getScriptTag()
    ...
    pt.getSignatureHtml = function () {
        ...
        return '<a href="' + pkg.homepage + '">' + pkg.name + '</a>@' + pkg.version
    ...
    pt.createNotFoundHtml = function (pathname) {
        ...
        return buf + this.getSignatureHtml()
